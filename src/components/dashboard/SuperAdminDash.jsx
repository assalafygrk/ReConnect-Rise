...
import { Database, Users, ShieldCheck, Layers, Fingerprint, ActivitySquare, Crown, AlertCircle, CheckCircle2, TrendingUp, Wallet, BadgeCheck, UserPlus, UserMinus, Clock, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 space-y-6 px-4">
      ...
      <div className="relative bg-white dark:bg-[#0A2540] rounded-[2rem] p-7 md:p-12 text-[#1A1A2E] dark:text-white shadow-xl shadow-black/5 dark:shadow-2xl border border-black/5 dark:border-white/10 overflow-hidden group transition-all duration-500">
        ...
        <div className="relative z-10 flex flex-col xl:flex-row justify-between gap-8">
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0">
            <div onClick={() => nav('/settings')} className="bg-gray-50 dark:bg-white/5 p-5 rounded-2xl border border-black/5 dark:border-white/10 space-y-1 backdrop-blur-md cursor-pointer hover:-translate-y-0.5 transition-all">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Database} label="Global Treasury" value={fmt(d.poolBalance)} color="#3B82F6" onClick={() => nav('/contributions')} />
        <StatCard icon={TrendingUp} label="Goal Velocity" value={`${goalPct}%`} sub="of Target" color="#F5A623" trend={goalPct > 50 ? +8 : -3} onClick={() => nav('/contributions')} />
        <StatCard icon={CheckCircle2} label="Paid / Total" value={`${d.totalPaid || 0} / ${d.totalMembers || 0}`} color="#10B981" onClick={() => nav('/contributions')} />
        <StatCard icon={Layers} label="Unpaid Risk" value={d.totalUnpaid || 0} sub="Delinquent" color="#F43F5E" onClick={() => nav('/members')} />
      </div>

      {/* Role Breakdown (super_admin exclusive) */}
      {isSA && d.roleBreakdown && (
        <div onClick={() => nav('/members')} className="bg-white dark:bg-[#111827] border border-black/5 dark:border-white/10 rounded-[1.5rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] cursor-pointer hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
          <h3 className="text-sm font-black uppercase tracking-[0.15em] text-[#1A1A2E] dark:text-white/90 mb-4 flex items-center justify-between"><span className="flex items-center gap-2"><Crown size={16} className="text-[#E8820C]" /> Network Roster Breakdown</span><ArrowRight size={14} className="text-black/20 dark:text-white/20" /></h3>
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard icon={UserPlus} label="Active" value={d.activeMembers || 0} color="#10B981" onClick={() => nav('/members')} />
        <StatCard icon={Clock} label="Pending" value={d.pendingMembers || 0} color="#F59E0B" onClick={() => nav('/members')} />
        <StatCard icon={UserMinus} label="Suspended" value={d.suspendedMembers || 0} color="#F43F5E" onClick={() => nav('/members')} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2">
          <ActivityFeed transactions={d.recentTransactions || []} title="Real-time Relay" accentColor="#3B82F6" onSelect={(tx) => nav('/wallet')} />
        </div>
      </div>
      ...
      {d.recentMembers?.length > 0 && (
        <div className="bg-white dark:bg-[#111827] border border-black/5 dark:border-white/10 rounded-[1.5rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          <h3 className="text-sm font-black uppercase tracking-[0.15em] text-[#1A1A2E] dark:text-white/90 mb-4 flex items-center justify-between"><span className="flex items-center gap-2"><Users size={16} className="text-[#3B82F6]" /> Recent Registrations</span><button onClick={() => nav('/members')} className="text-[9px] font-black uppercase tracking-widest text-[#3B82F6] flex items-center gap-1 hover:underline">View All <ArrowRight size={10} /></button></h3>
          <div className="space-y-2">
            {d.recentMembers.map(m => (
              <div key={m.id} onClick={() => nav(`/members/${m.id}`)} className="flex items-center gap-3 p-3 rounded-xl border border-black/[0.03] dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors cursor-pointer hover:-translate-x-0.5">
      ...
      {d.topContributors?.length > 0 && (
        <div className="bg-white dark:bg-[#111827] border border-black/5 dark:border-white/10 rounded-[1.5rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
          <h3 className="text-sm font-black uppercase tracking-[0.15em] text-[#1A1A2E] dark:text-white/90 mb-4 flex items-center justify-between"><span className="flex items-center gap-2"><BadgeCheck size={16} className="text-[#F5A623]" /> Top Contributors</span><button onClick={() => nav('/contributions')} className="text-[9px] font-black uppercase tracking-widest text-[#F5A623] flex items-center gap-1 hover:underline">View All <ArrowRight size={10} /></button></h3>
          <div className="space-y-2">
