...
import { Users, Crown, Target, CheckCircle2, ShieldCheck, HeartHandshake, HandCoins, AlertCircle, UserPlus, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto pb-24 space-y-6 px-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Users} label="Members" value={d.totalMembers || 0} color="#8B5CF6" onClick={() => nav('/members')} />
        <StatCard icon={Target} label="Goal Progress" value={`${goalPct}%`} color="#C084FC" sub={fmt(d.poolBalance)} trend={goalPct > 50 ? +5 : -2} onClick={() => nav('/contributions')} />
        <StatCard icon={CheckCircle2} label="Paid This Month" value={d.totalPaid || 0} color="#10B981" sub={`${d.totalUnpaid || 0} unpaid`} onClick={() => nav('/contributions')} />
        <StatCard icon={ShieldCheck} label="Pending Requests" value={(stats.pendingRequests || 0) + (stats.welfareApprovedRequests || 0)} color="#F5A623" sub="Welfare + Loans" onClick={() => nav('/welfare')} />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div onClick={() => nav('/members')} className="bg-white dark:bg-[#111827] border border-black/5 dark:border-white/10 rounded-2xl p-5 shadow-sm cursor-pointer hover:-translate-y-0.5 hover:shadow-md transition-all">
          <div className="flex items-center gap-3 mb-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2">
          <ActivityFeed transactions={d.recentTransactions || []} title="Member Activity" accentColor="#8B5CF6" onSelect={() => nav('/wallet')} />
        </div>
      </div>
      ...
      {d.recentMembers?.length > 0 && (
        <div className="bg-white dark:bg-[#111827] border border-black/5 dark:border-white/10 rounded-[1.5rem] p-6 shadow-sm">
          <h3 className="text-sm font-black uppercase tracking-[0.15em] text-[#1A1A2E] dark:text-white/90 mb-4 flex items-center justify-between"><span className="flex items-center gap-2"><Users size={16} className="text-[#8B5CF6]" /> Latest Members</span><button onClick={() => nav('/members')} className="text-[9px] font-black uppercase tracking-widest text-[#8B5CF6] flex items-center gap-1 hover:underline">View All <ArrowRight size={10} /></button></h3>
          <div className="space-y-2">
            {d.recentMembers.map(m => (
              <div key={m.id} onClick={() => nav(`/members/${m.id}`)} className="flex items-center gap-3 p-3 rounded-xl border border-black/[0.03] dark:border-white/5 cursor-pointer hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
