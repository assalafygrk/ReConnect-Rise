...
import {
    ...
    Search, Loader2, ClipboardCheck,
    ShieldAlert, BadgeCheck,
    Printer, TrendingUp, Inbox, Fingerprint, History, HeartHandshake,
    ArrowRight
} from 'lucide-react';
...
export default function WelfarePage() {
    ...
    const [publicTab, setPublicTab] = useState('transparency');
    ...
    const loadData = async () => {
        ...
        try {
            ...
            toast.error('Failed to load welfare registry');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setRequests((prev) => [{ ...data, id: data._id, submittedBy: user?.name || 'Member', date: new Date().toISOString() }, ...prev]);
    ...
    const myHistory = requests.filter(r => r.submittedBy === user?.name || r.user === user?._id);

    const myPendingCount = requests.filter(r => (r.submittedBy === user?.name || r.user === user?._id) && r.status === 'pending').length;
                                        ...
                                        return (
                                            <tr key={req.id} onClick={() => setSelectedRequest(req)} className="group hover:bg-gray-50 transition-colors cursor-pointer">
                                                <td className="px-10 py-6">
                                                    <span className="text-[11px] font-black text-black/20 font-mono tracking-tighter">#RR-W-{String(req.id).slice(-4)}</span>
                                                </td>
                                            ...
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div>
                                                    <div className="flex items-center gap-3 mb-2">
                                                        ...
                                                        <span className="text-[10px] font-black text-black/20 font-mono">#REQ-{String(req.id).slice(-4)}</span>
                                                    </div>
                        ...
                        <div className="flex items-center justify-between mb-12 sticky top-0 bg-white z-10 py-2">
                            <div className="space-y-2">
                                ...
                                <p className="text-[10px] text-black/30 font-black tracking-[0.4em] flex items-center gap-2 uppercase">
                                    <ClipboardCheck size={14} className="text-[#E8820C]" /> Protocol #RR-W-{String(selectedRequest.id).slice(-4)}
                                </p>
                            </div>
                                ...
                                <div className="bg-gray-50/50 p-8 rounded-[2.5rem] border border-black/5 space-y-6 shadow-inner">
                                    ...
                                    <div className="flex items-center gap-5">
                                        <div className="w-16 h-16 bg-[#1A1A2E] rounded-[1.5rem] flex items-center justify-center text-[#F5A623] text-xl font-serif italic shadow-xl">
                                            {(selectedRequest.submittedBy || 'U').split(' ').map(n => n[0]).join('')}
                                        </div>
                                        <div>
                                            <p className="text-lg font-black text-[#1A1A2E]">{selectedRequest.submittedBy || 'Unknown'}</p>
