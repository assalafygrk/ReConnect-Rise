...
const STATUS = {
    ...
    welfare_approved: { label: 'At Group Leader', color: 'text-blue-600',    bg: 'bg-blue-50',    icon: ArrowRight },
    leader_approved:  { label: 'At Treasurer',    color: 'text-indigo-600',  bg: 'bg-indigo-50',  icon: ArrowRight },
    approved:         { label: 'Disbursed',        color: 'text-emerald-600', bg: 'bg-emerald-50', icon: CheckCircle2 },
...
export default function WelfarePage() {
    ...
    if (loading) return (
        <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-xs font-black uppercase tracking-widest text-black/30">Loading Welfare Centre...</p>
        </div>
    );
        ...
        <div className="max-w-7xl mx-auto pb-20 space-y-8 px-4">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-white/5 rounded-xl border border-white/10 text-[#F5A623]"><Shield size={20} /></div>
                            <h1 className="text-3xl md:text-4xl font-serif font-black tracking-tight">Welfare Centre</h1>
                        </div>
                        <p className="text-white/40 text-sm max-w-md">3-stage approval: Welfare Officer → Group Leader → Treasurer</p>
                        <div className="flex gap-4 text-center pt-2">
                            {[['Pending', welfareQueue.length, 'text-amber-400'], ['At Leader', leaderQueue.length, 'text-blue-400'], ['At Treasurer', treasurerQueue.length, 'text-indigo-400']].map(([l,v,c]) => (
                                <div key={l} className="bg-white/5 rounded-xl px-4 py-2 border border-white/10">
                    ...
                    <button onClick={() => setShowForm(true)}
                        className="bg-white text-[#1A1A2E] px-8 py-5 rounded-[2rem] font-black text-xs uppercase tracking-widest flex items-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-2xl">
            ...
            {isWelfareOfficer && welfareQueue.length > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-[2rem] p-6 space-y-4">
                    <h3 className="text-sm font-black text-amber-800 uppercase tracking-widest flex items-center gap-2"><ShieldAlert size={16} /> Your Queue — Welfare Officer ({welfareQueue.length})</h3>
                    {welfareQueue.map(r => (
                        <div key={r._id} className="bg-white rounded-2xl p-5 border border-amber-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <p className="font-black text-[#1A1A2E]">{r.member} <span className="text-black/30 font-normal">— {r.type}</span></p>
                                <p className="text-xs text-black/40 mt-0.5 line-clamp-1">{r.description}</p>
                                <p className="text-sm font-black text-[#E8820C] mt-1">{fmt(r.amount)} · <span className="text-[10px] font-bold text-black/30">{r.paymentMethod}</span></p>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={() => doAction('welfare', r._id, 'approve')} disabled={actionLoading}
                                    className="px-5 py-3 bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50">
                                ...
                                <button onClick={() => doAction('welfare', r._id, 'decline')} disabled={actionLoading}
                                    className="px-5 py-3 bg-red-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-700 active:scale-95 transition-all disabled:opacity-50">
                                    Decline
                                </button>
                            </div>
                        </div>
            ...
            {isLeader && leaderQueue.length > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-[2rem] p-6 space-y-4">
                    <h3 className="text-sm font-black text-blue-800 uppercase tracking-widest flex items-center gap-2"><ShieldAlert size={16} /> Your Queue — Group Leader ({leaderQueue.length})</h3>
                    {leaderQueue.map(r => (
                        <div key={r._id} className="bg-white rounded-2xl p-5 border border-blue-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <p className="font-black text-[#1A1A2E]">{r.member} <span className="text-black/30 font-normal">— {r.type}</span></p>
                                <p className="text-xs text-black/40 mt-0.5 line-clamp-1">{r.description}</p>
                                <p className="text-sm font-black text-[#E8820C] mt-1">{fmt(r.amount)} · <span className="text-[10px] font-bold text-black/30">{r.paymentMethod}</span></p>
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => doAction('leader', r._id, 'approve')} disabled={actionLoading}
                                    className="px-5 py-3 bg-indigo-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50">
            ...
            {isTreasurer && treasurerQueue.length > 0 && (
                <div className="bg-emerald-50 border border-emerald-200 rounded-[2rem] p-6 space-y-4">
                    <h3 className="text-sm font-black text-emerald-800 uppercase tracking-widest flex items-center gap-2"><ShieldAlert size={16} /> Your Queue — Treasurer ({treasurerQueue.length})</h3>
                    {treasurerQueue.map(r => (
                        <div key={r._id} className="bg-white rounded-2xl p-5 border border-emerald-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                <p className="font-black text-[#1A1A2E]">{r.member} <span className="text-black/30 font-normal">— {r.type}</span></p>
                                <p className="text-xs text-black/40 mt-0.5 line-clamp-1">{r.description}</p>
                                <p className="text-sm font-black text-[#E8820C] mt-1">{fmt(r.amount)} · <span className="text-[10px] font-bold text-black/30 uppercase">{r.paymentMethod}</span></p>
                            </div>
                            <div className="flex gap-3">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-black/5 overflow-hidden">
                <div className="p-6 border-b border-black/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <h3 className="text-lg font-serif font-bold text-[#1A1A2E]">{isPrivileged ? 'All Requests' : 'My Applications'}</h3>
                    <div className="flex items-center gap-3">
                        ...
                        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
                            className="bg-gray-50 border-2 border-transparent focus:border-[#E8820C] rounded-2xl px-4 py-2.5 text-xs font-bold outline-none">
                ...
                <div className="divide-y divide-black/5">
                    {filtered.map(r => {
                        ...
                        const Icon = st.icon;
                        return (
                            <div key={r._id} className="p-6 hover:bg-gray-50 transition-colors flex items-center justify-between gap-4 cursor-pointer" onClick={() => setSelected(r)}>
                                <div className="flex items-center gap-4">
                                    <div className={`p-2.5 rounded-xl ${st.bg} ${st.color}`}><Icon size={18} /></div>
                                    <div>
                                        <p className="font-black text-sm text-[#1A1A2E]">{isPrivileged ? r.member : r.type + ' Support'}</p>
                                        <p className="text-[10px] text-black/30 font-bold uppercase tracking-widest">{st.label} · {dayjs(r.createdAt || r.date).format('DD MMM YYYY')}</p>
                                    </div>
                                </div>
                    ...
                    {filtered.length === 0 && (
                        <div className="py-16 text-center">
                            <p className="text-xs font-black uppercase tracking-widest text-black/20">No requests found</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative animate-in zoom-in-95 duration-300">
                        <div className="flex items-center justify-between mb-8">
                            <h3 className="text-2xl font-serif font-black text-[#1A1A2E]">Apply for Support</h3>
                            <button onClick={() => setShowForm(false)} className="p-3 bg-gray-50 rounded-2xl text-black/20 hover:text-black transition-all"><X size={20} /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-black/40 uppercase tracking-widest">Category</label>
                                    <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}
                                        className="w-full bg-gray-50 border-2 border-transparent focus:border-[#E8820C] rounded-2xl px-4 py-4 text-sm font-bold outline-none">
                                ...
                                <div className="flex gap-3">
                                    {['wallet','cash'].map(m => (
                                        <button type="button" key={m} onClick={() => setForm({ ...form, paymentMethod: m })}
                                            className={`flex-1 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${form.paymentMethod === m ? 'bg-[#1A1A2E] text-white' : 'bg-gray-50 text-black/40'}`}>
                                            {m}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-black/40 uppercase tracking-widest">Description</label>
            ...
            {showDeclineBox && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white w-full max-w-sm rounded-[2rem] p-8 shadow-2xl relative animate-in zoom-in-95 duration-200">
                        <h4 className="text-lg font-serif font-black text-[#1A1A2E] mb-4">Decline Reason</h4>
                        <textarea value={declineReason} onChange={e => setDeclineReason(e.target.value)} rows={3}
                            className="w-full bg-gray-50 border-2 border-transparent focus:border-red-400 rounded-2xl px-5 py-4 text-sm font-medium outline-none resize-none mb-4"
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => { setShowDeclineBox(false); setPendingAction(null); setDeclineReason(''); }}
                                className="flex-1 py-3 bg-gray-100 text-black/50 rounded-xl font-black text-xs uppercase tracking-widest">Cancel</button>
