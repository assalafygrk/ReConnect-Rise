...
import {
    Plus, Search, RotateCcw, Vote as VoteIcon,
    CheckCircle2, Activity, ShieldCheck, Zap, MoreHorizontal
} from 'lucide-react';
...
export default function VotesPage() {
    ...
    const loadVotes = async () => {
        ...
        try {
            ...
            toast.error('Governance registry unreachable');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreate = async (voteData) => {
        try {
            ...
            toast.error('Ballot authorization failed');
        }
    };
    ...
    const handleClose = async (voteId) => {
        try {
            ...
            setVotes(prev => prev.map(v => v._id === voteId ? { ...v, status: 'closed' } : v));
    ...
    return (
        <div className="max-w-7xl mx-auto pb-20 space-y-12 px-4">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                <div className="space-y-4">
                    <div className="flex items-center gap-3 text-[10px] font-black text-[#E8820C] uppercase tracking-[0.4em]">
                        <VoteIcon size={16} /> Strategic Consensus
                    </div>
            ...
            {config.abstentionPolicy && (
                <div className="bg-blue-50 border border-blue-200 p-6 rounded-[2rem] flex items-center gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    { label: 'Participation', value: 'Audited', icon: ShieldCheck, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                    { label: 'Finalized', value: votes.filter(v => v.status === 'closed').length, icon: CheckCircle2, color: 'text-blue-600', bg: 'bg-blue-50' },
                    { label: config.quorumLabel || 'Quorum Status', value: `Min ${config.quorumThreshold || 50}%`, icon: Zap, color: 'text-amber-600', bg: 'bg-amber-50' },
                ].map((stat, i) => (
                    <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-black/5 shadow-sm space-y-4 group hover:shadow-xl transition-all duration-500">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                <div className="flex-1 relative group">
                    ...
                    <input
                        ...
                        className="w-full bg-white border border-black/5 rounded-[1.5rem] pl-16 pr-6 py-5 text-sm font-bold text-[#1A1A2E] shadow-sm outline-none focus:ring-4 focus:ring-[#E8820C]/10 transition-all placeholder:text-black/10"
                    />
                </div>
                ...
                <div className="flex items-center gap-3 p-2 bg-white rounded-[1.5rem] border border-black/5 shadow-sm">
                    {['all', 'open', 'closed'].map((f) => (
                        <button
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`px-10 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all ${filter === f ? 'bg-[#1A1A2E] text-white shadow-xl' : 'text-black/30 hover:bg-gray-50'
                                }`}
                        >
                            {f}
                        </button>
                ...
                <button
                    onClick={loadVotes}
                    className="p-5 bg-white border border-black/5 text-black/20 hover:text-[#E8820C] rounded-[1.5rem] shadow-sm transition-all active:scale-95"
                >
                    <RotateCcw size={24} />
                </button>
            </div>
            ...
            {filteredVotes.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-40 bg-white rounded-[4rem] border-2 border-dashed border-black/5 space-y-8 text-center shadow-inner">
                    <div className="w-32 h-32 bg-gray-50 rounded-[3rem] flex items-center justify-center text-black/5">
                        <VoteIcon size={80} />
                    </div>
                    <div className="space-y-2">
                        <h3 className="text-3xl font-serif font-black text-[#1A1A2E]">Consensus Archive Clear</h3>
                        <p className="text-sm text-black/30 font-medium max-w-sm mx-auto">
                            No active or historical governance records match your criteria.
                        </p>
                    </div>
                </div>
            )}
